        ...
        const fetchNotifSettings = async () => {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    if (typeof data.notifWorkout === "boolean") {
                        setNotifWorkout(data.notifWorkout);
                        await AsyncStorage.setItem("notif_workout", String(data.notifWorkout));
                    }
                    if (typeof data.notifPersonalWorkout === "boolean") {
                        setNotifPersonalWorkout(data.notifPersonalWorkout);
                    }
                    if (typeof data.notifDiet === "boolean") {
                        setNotifDiet(data.notifDiet);
                        await AsyncStorage.setItem("notif_diet", String(data.notifDiet));
                    }
                    if (typeof data.notifNewMember === "boolean") {
                        setNotifNewMember(data.notifNewMember);
                        await AsyncStorage.setItem("notif_new_member", String(data.notifNewMember));
                    }
                }
            } catch {}
            setLoading(false);
    ...
    return (
        <View style={{ flex: 1, backgroundColor: "#fff" }}>
            ...
            <View style={{
                ...
                <Text style={{ fontSize: 20, fontWeight: "800", color: Colors.text }}>알림 설정</Text>
            </View>
...
function SwitchRow({
    label, desc, value, onValueChange, bold, disabled,
}: {
    label: string;
    desc?: string;
    value: boolean;
