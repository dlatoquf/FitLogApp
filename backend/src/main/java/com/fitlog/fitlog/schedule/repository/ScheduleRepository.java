...
""")
    ...
    @Query("SELECT DISTINCT s.trainer FROM Schedule s WHERE s.date = :date AND s.status = 'CONFIRMED' AND s.sessionType NOT IN ('PERSONAL_SCHEDULE', 'PERSONAL_WORKOUT')")
    List<Trainer> findTrainersWithConfirmedScheduleOn(@Param("date") java.time.LocalDate date);

    // 특정 트레이너의 특정 날짜 CONFIRMED 수업 수
    @Query("SELECT COUNT(s) FROM Schedule s WHERE s.trainer = :trainer AND s.date = :date AND s.status = 'CONFIRMED' AND s.sessionType NOT IN ('PERSONAL_SCHEDULE', 'PERSONAL_WORKOUT')")
