...
/**
 * 미연동 회원 중 PT 종료 후 37일(유예 7일 + 비활성 30일) 경과 시 자동 삭제
 */
@Component
public class ManualMemberCleanupScheduler {
    ...
    // 매일 새벽 4시 30분 — 피티 종료 37일 경과 미연동 회원 자동 삭제
    @Scheduled(cron = "0 30 4 * * *")
    @Transactional
    public void cleanupPtEndedManualMembers() {
        // ptEndedAt + 37일 <= 오늘 → 삭제 대상 (유예 7일 + 비활성 30일)
        LocalDate cutoff = LocalDate.now().minusDays(37);
        ...
        for (ManualMember m : toDelete) {
            ...
            System.out.println("피티종료 37일 자동삭제: manualMemberId=" + m.getId() + " name=" + m.getName());
        }
        if (!toDelete.isEmpty()) {
